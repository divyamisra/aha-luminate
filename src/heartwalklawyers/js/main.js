  ...
  $(document).ready(function ($) {
          ...
          success: function (response) {
            if (response.getCompaniesResponse.totalNumberResults === '0') {
              ...
              $(companies).each(function (i, company) {
                $('.js__company-results-rows').append(
                  '<tr' +
                    (i > 10 ? ' class="d-none"' : '') +
                    '><td><a href="' +
                    company.companyURL +
                    ...
                    '</a></td><td class="col-cta"><a href="' +
                    company.companyURL +
                    '" aria-label="Visit page for ' +
                    company.companyName +
                    '" class="btn-rounded btn-primary btn-block">View</a></td></tr>'
                )
              })
    ...
    $('.js--header-zip-search').on('submit', function (e) {
      ...
      window.location.href =
        ...
        'SPageServer/?pagename=HeartWalk_LHH_Search&search_type=event&cross_event=' +
        (evID ? 'false' : 'true') +
        (evID ? '&fr_id=' + evID : '') +
        '&zip=' +
        zipSearched
    ...
    $('.js--header-walker-search').on('submit', function (e) {
      ...
      window.location.href =
        luminateExtend.global.path.secure +
        'SPageServer/?pagename=HeartWalk_LHH_Search&search_type=walker&cross_event=false&fr_id=' +
        evID +
        (firstName ? '&first_name=' + firstName : '') +
        (lastName ? '&last_name=' + lastName : '')
    })
    ...
    $('.js--header-team-search').on('submit', function (e) {
      ...
      window.location.href =
        luminateExtend.global.path.secure +
        'SPageServer/?pagename=HeartWalk_LHH_Search&search_type=team&cross_event=' +
        (evID ? 'false' : 'true') +
        (evID ? '&fr_id=' + evID : '') +
        '&team_name=' +
        teamName
          ...
          success: function (response) {
            if (response.getTeamraisersResponse.totalNumberResults > '0') {
              ...
              $(events).each(function (i, event) {
                ...
                var eventDate = luminateExtend.utils.simpleDateFormat(event.event_date, 'EEEE, MMMM d, yyyy')
                ...
                var eventRow =
                  ...
                  eventType +
                  ' Event</span><span class="event-date d-block">' +
                  eventDate +
                  '</span></div><a href="' +
                  greetingUrl +
        ...
        callback: {
          success: function (response) {
            if (response.getTeamraisersResponse.totalNumberResults > '0') {
              ...
              $(events).each(function (i, event) {
                var eventDate = luminateExtend.utils.simpleDateFormat(event.event_date, 'EEE, MMM d, yyyy')
                var eventTimestamp = new Date(event.event_date)
                var eventStatus = event.status
                var acceptsRegistration = event.accepting_registrations
                ...
                if (screenWidth >= 768) {
                  var eventRow =
                    ...
                    (acceptsRegistration === 'true'
                      ? '<a href="SPageServer/?pagename=lhh_register&fr_id=' +
                        event.id +
    ...
    cd.getTopParticipantsSteps = function (eventId) {
      var motionApiUrl =
        'https://' +
        motion_urlPrefix +
        '.boundlessfundraising.com/mobiles/' +
        motionDb +
        '/getMotionActivityRoster?event_id=' +
        motion_event +
        '&roster_type=participant&list_size=5000'
          ...
          if (response.activities != undefined) {
            $(response.activities).each(function () {
              if (participantsFound < 5) {
                ...
                var topWalkerHtml =
                  '<li><div class="d-flex"><div class="flex-grow-1"><a title="' +
                  participantName +
                  ' Minutes" href="' +
                  participantPage +
                  '">' +
                  participantName +
                  '</a></div><div class="raised"><strong>' +
                  steps +
                  '</strong><br/>Minutes</div></div></li>'
    ...
    cd.getTopTeamsSteps = function (eventId) {
      var motionApiUrl =
        ...
        '/getMotionActivityRoster?event_id=' +
        motion_event +
        '&roster_type=team&list_size=5000'
      var teamsFound = 0
    ...
    cd.getTopCompaniesSteps = function (eventId) {
      var motionApiUrl =
        ...
        '/getMotionActivityRoster?event_id=' +
        motion_event +
        '&roster_type=company&list_size=5000'
    ...
    cd.initializeTeamRosterTable = function () {
      ...
      $('#team-roster_filter input[type="search"]')
        ...
        .after(
          '<div class="input-group-append"><button class="btn btn-primary btn-outline-secondary" type="button">Search <i class="fas fa-search"></i></button></div>'
        )
    ...
    if ($('body').is('.pg_entry')) {
      ...
      $('.js--greeting-team-search-form').on('submit', function (e) {
        ...
        window.location.href =
          luminateExtend.global.path.secure +
          'SPageServer/?pagename=HeartWalk_LHH_Search&search_type=team&cross_event=false&fr_id=' +
          evID +
          '&team_name=' +
          teamName
      })
            ...
            success: function (response) {
              ...
              if (response.getPersonalVideoUrlResponse.videoUrl) {
                ...
                videoEmbedHtml =
                  ...
                  '" title="American Heart Association Heart Walk Video" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>'
              } else {
                // TODO - show default video
                if (isProd) {
                  if (evID === 5612 || evID === 4854 || evID === 5731) {
                    videoEmbedHtml =
                      '<iframe class="embed-responsive-item" src="https://www.youtube.com/embed/TnjvKjkANPI?wmode=opaque&amp;rel=0&amp;showinfo=0" title="American Heart Association Heart Walk Video" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>'
                  } else {
                    videoEmbedHtml =
                      '<iframe class="embed-responsive-item" src="https://www.youtube.com/embed/Z0LO1pgspH8?wmode=opaque&amp;rel=0&amp;showinfo=0" title="American Heart Association Heart Walk Video" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>'
                  }
                } else {
                ...
                $(participants).each(function (i, participant) {
                  ...
                  $('#team-roster tbody').append(
                    '<tr class="' +
                      ...
                      participant.name.last +
                      '</a>' +
                      (participant.aTeamCaptain === 'true' ? ' <span class="coach">- Coach</span>' : '') +
                      '</td><td class="raised" data-sort="' +
                      participantRaisedFormmatted +
                      '"><span><strong>$' +
                      participantRaisedFormmatted +
        ...
        $('a:contains(Donat)').on('click', function (e) {
          ...
          if (!$(this).hasClass('js--team-member-donate')) {
            if ($(this).next('.paymentSelType').length > 0) {
              ...
              var html =
                "<div class='paymentSelType text-center' style='padding-top:10px;'>" +
                ...
                "<a href='" +
                default_path +
                '/site/SPageNavigator/heartwalk_donate_amazon.html?FR_ID=' +
                fr_id +
                '&mfc_pref=T&PROXY_ID=' +
                px +
                '&PROXY_TYPE=' +
                ...
                "<a href='" +
                default_path +
                '/site/SPageNavigator/heartwalk_donate_googlepay.html?FR_ID=' +
                fr_id +
                '&mfc_pref=T&PROXY_ID=' +
                px +
                '&PROXY_TYPE=' +
                ...
                "<a href='" +
                default_path +
                '/site/SPageNavigator/heartwalk_donate_applepay.html?FR_ID=' +
                fr_id +
                '&mfc_pref=T&PROXY_ID=' +
                px +
                '&PROXY_TYPE=' +
                ...
                "<a href='" +
                default_path +
                '/site/SPageNavigator/heartwalk_donate_venmo.html?FR_ID=' +
                fr_id +
                '&mfc_pref=T&PROXY_ID=' +
                px +
                '&PROXY_TYPE=' +
